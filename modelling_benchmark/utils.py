...
def load_data(hla_representation='simple',species_representation='simple',
    ...
    #encode hla
    x_train_hla=encode_hla(data.mhc.values,t_data.mhc.values,hla_representation)
    x_test_hla=encode_hla(data.mhc.values,b_data.mhc.values,hla_representation)
    
    #encode species
    x_train_species=encode_hla(data.species.values,t_data.species.values,species_representation)
    x_test_species=encode_hla(data.species.values,b_data.species.values,species_representation)
    ...
    x_train=np.column_stack([x_train_species,x_train_hla,x_train_seq,t_data.peptide_length.values])
    x_test=np.column_stack([x_test_species,x_test_hla,x_test_seq,b_data.peptide_length.values])
    ...
    #classify y
    y_train[y_train<np.log10(500)]=0
    y_train[y_train>=np.log10(500)]=1
    
    y_test[y_test<np.log10(500)]=0
    y_test[y_test>=np.log10(500)]=1
...
def encode_hla(x_all,x_in,representation):
...
def encode_species(x_in,representation):
    """Encode string species type to numbers."""
    if representation=='simple':
        le = preprocessing.LabelEncoder()
        le.fit(x_all)
        x_out=le.transform( x_in )
